package com.trimh.trimphmvp.progress;
...
//        mPaint.setStrokeCap(Paint.Cap.ROUND);
    ...
    private void initData() {
        ...
        mPaint.setStrokeWidth((float) (mCircleRadius * 1.5));//设置画笔的宽度
        Log.e("trimph------------", "mLineLength::" + mLineLength + " mCircleRadius:" + mCircleRadius + "StrokeWidth:" + mCircleRadius * 2);
        Log.e("trimph------------", "mEntireLineLength::" + mEntireLineLength);
        mStep = 0;
    }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        switch (mStep % 4) {
            case 0:
                for (int i = 0; i < mColors.length - 2; i++) {
                    mPaint.setColor(mColors[i]);
                    //
                    drawCRLC(canvas, mWidth / 2 - mEntireLineLength / 2.2f,
            ...
            case 1:
                for (int i = 0; i < mColors.length; i++) {
                    mPaint.setColor(mColors[i]);
                    drawCR(canvas, mWidth / 2 - mEntireLineLength / 2.2f,
                            mHeight / 2 + mEntireLineLength, mPaint,
                            mCanvasAngle + i * 90);
                }
                break;
                /*
...
//                break;/**/
    ...
    private void drawCRLC(Canvas canvas, float startX, float startY, float stopX, float stopY, @NonNull Paint paint, int rotate) {
        Log.e("trimph----------------", "startX" + startX + " startY:" + startY + "stopx:" + stopX + " stopy::" + stopY);
        Log.e("trimph----------------", "mWidth/2:" + mWidth / 2 + " mHeight/2:" + mHeight / 2);
    ...
    private void startCRLCAnim() {
        ...
        lineWidthAnim.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
            ...
            public void onAnimationUpdate(ValueAnimator animation) {
                ...
                Log.e("trimph", "----------------------------mLineLength" + mLineLength);
                invalidate();
            }
        });
